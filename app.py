...
class CleaningRobot:
    ...
    def _desired_vel(self):
        ...
        wall_x += wf(self.x)
        wall_x -= wf(ROOM_W - self.x)
        wall_y += wf(self.y)
        wall_y -= wf(ROOM_H - self.y)
...
class RoomCleaningSimulation:
    def __init__(self):
        ...
        self.robot = CleaningRobot(2.0, 2.0, self.obstacles, self.grid_cols, self.grid_rows)
...
with st.sidebar:
    ...
    steps = st.slider("Langkah per Frame", 1, 8, st.session_state.steps_per_frame,
                      help="Lebih tinggi = simulasi lebih cepat (tapi lebih berat CPU)")
    st.session_state.steps_per_frame = steps

    ref_ms = st.slider("Refresh (ms)", 30, 300, st.session_state.refresh_ms, 10,
